...
local function ui_notify(str)
    if vim.g.ui_notifications_enabled then
        require("flitzfiete.utils").notify(str)
    end
end
...
function M.toggle_copilot()
    ...
    vim.cmd.Copilot(action)
    ui_notify(string.format("Copilot is now %sd", action))
end
...
--   require("flitzfiete.utils").set_url_match()
-- end
